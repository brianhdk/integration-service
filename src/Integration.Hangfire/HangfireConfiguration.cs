...
namespace Vertica.Integration.Hangfire
{
	public class HangfireConfiguration : IInitializable<IWindsorContainer>
	{
		...
		public HangfireConfiguration Configuration(Action<IGlobalConfiguration> configuration)
		{
            ...
            return Configuration((inner, _) => configuration(inner));
		}
		...
		public HangfireConfiguration WithServerOptions(Action<BackgroundJobServerOptions> serverOptions)
		{
            ...
            return WithServerOptions((inner, _) => serverOptions(inner));
		}

		public HangfireConfiguration WithServerOptions(Action<BackgroundJobServerOptions, IKernel> serverOptions)
		{
			if (serverOptions == null) throw new ArgumentNullException(nameof(serverOptions));

            _configuration.Add(serverOptions);
		...
		/// <summary>
		/// Clears all registered BackgroundProcesses.
		/// </summary>
	        ...
	        private T Ensure<T>(Func<JobStorage, T> withJobStorage)
	        {
	            if (!_initialized)
	            {
	                lock (this)
	                {
	                    if (!_initialized)
	                    {
	                        foreach (Action<IGlobalConfiguration, IKernel> configurer in _configuration.Configurations)
