using System;
using System.Reflection;
using Castle.Facilities.TypedFactory;
using Castle.MicroKernel;
using Microsoft.WindowsAzure.Storage.Blob;
using Vertica.Integration.Azure.Infrastructure.BlobStorage;

namespace Vertica.Integration.Azure.Infrastructure.Castle.Windsor
{
	internal class AzureBlobStorageSelector<TConnection> : ITypedFactoryComponentSelector
		where TConnection : Connection
	{
		private readonly Connection _connection;

        public AzureBlobStorageSelector(Connection connection)
		{
			if (connection == null) throw new ArgumentNullException("connection");

			_connection = connection;
		}

		public Func<IKernelInternal, IReleasePolicy, object> SelectComponent(MethodInfo method, Type type, object[] arguments)
		{
			return (kernel, policy) =>
			{
                if (method.ReturnType == typeof(CloudBlobClient))
                    return kernel.Resolve(_connection.CloudBlobClient, method.ReturnType);

				throw new NotSupportedException(
					String.Format("Method '{0}' in type '{1}' not supported.", method.Name, type.FullName));
			};
		}
	}
}